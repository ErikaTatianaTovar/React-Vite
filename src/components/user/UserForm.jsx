    ...
    const handleSubmit = async (e) => {
        ...
        if (response.data.success == "error") {
            Swal. fire({
                position: "top-end",
                icon: "error",
                title: "El usuario no pudo ser registrado, por favor verifique la información",
        ...
        Swal. fire({
            ...
            navigate('/user') //hacer la redirección
        });
    }
        }catch (error){
        ...
        <div className="max-w-md w-full mx-auto px-5 py-5">
            <form onSubmit={handleSubmit} className="shadow-md rounded pt-6 pb-10 mb-4 px-10">
                ...
                <div className="mb-4">
                    ...
                    <input type="text" required name="lastname" placeholder="Lastname" value={user.lastname}
                    className="shadow appearance-none border rounded w-full focus:shadow-outline" />
                </div>
                <div className="mb-4">
                    <label className="block text-gray-700 font-bold mb-2">Email</label>
                    <input type="email" required name="email" placeholder="Email" value={user.email}
                    className="shadow appearance-none border rounded w-full focus:shadow-outline" />
                </div>
                <div className="mb-4">
                    <label className="block text-gray-700 font-bold mb-2">Identification</label>
                    <input type="number" required name="id" placeholder="Identification" value={user.id}
                    className="shadow appearance-none border rounded w-full focus:shadow-outline" />
                </div>
                {params.id ? null :
                (<div className="mb-4">
